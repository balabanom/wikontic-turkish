...
def visualize_ontology_neighborhood(neighborhood: dict):
    """
    get_ontology_neighborhood() çıktısından pyvis grafı çizer.
    - Merkez node: mavi
    - Parent node'lar: turuncu
    - Property node'lar: yeşil (subject dir.) / mor (object dir.)
    - Merkez → parent: "is a" edge
    - Merkez → property: property label edge
    """
    net = Network(
    ...
    center_label = f"{center['label']}\n({center['id']})"
    net.add_node(center["id"], label=center_label, color="#4A90D9", size=25)

    # Parent node'lar
    for parent in neighborhood.get("parents", []):
        parent_label = f"{parent['label']}\n({parent['id']})"
        net.add_node(parent["id"], label=parent_label, color="#F5A623", size=18)
    ...
    for prop in neighborhood.get("properties", []):
        ...
        prop_label = f"{prop['label']}\n({prop['id']})\n{direction_label}"
        net.add_node(prop_node_id, label=prop_label, color=color, size=14, shape="box")
...
def render_transparency_panel(selected_run_id: str):
    """
    Verilen run_id için şeffaflık panelini render eder.
    Run metadata + 5 tab.
    """
    if not selected_run_id:
        st.info("Henüz extraction yapılmadı. Bir metin girin ve 'Extract and Visualize' butonuna tıklayın.")
        return
    ...
    with tab0:
        artifact = get_artifact(selected_run_id, "raw_llm_output")
        if artifact is None:
            st.warning("Bu stage için kayıt bulunamadı.")
        else:
            with st.expander("Ham LLM Çıktısı", expanded=True):
                st.code(artifact.get("text", ""), language="json")

    # ── Tab 1: Parse edilmiş triplets ─────────────────────────────────────────
    with tab1:
        artifact = get_artifact(selected_run_id, "parsed_triplets")
        if artifact is None:
            st.warning("Bu stage için kayıt bulunamadı.")
        else:
            triplets = artifact.get("triplets", [])
            count = artifact.get("count", len(triplets))
            st.caption(f"**{count} triplet** parse edildi")
            if triplets:
                df = pd.DataFrame(triplets)[["subject", "relation", "object"]]
    ...
    # ── Tab 2: Merge Log ──────────────────────────────────────────────────────
    with tab2:
        artifact = get_artifact(selected_run_id, "merge_map_entities")
        if artifact is None:
            st.warning("Bu stage için kayıt bulunamadı.")
        else:
            merges = artifact.get("merges", [])
            count = artifact.get("count", len(merges))
            if count == 0:
                st.info("Bu run'da hiçbir entity merge edilmedi.")
            else:
                st.caption(f"**{count} entity** merge edildi")
                df = pd.DataFrame(merges)
                cols = ["from", "to", "entity_type", "method"]
                existing_cols = [c for c in cols if c in df.columns]
                st.dataframe(df[existing_cols], use_container_width=True, hide_index=True)

    # ── Tab 3: Filtered Out ───────────────────────────────────────────────────
    with tab3:
        artifact = get_artifact(selected_run_id, "filtered_out")
        if artifact is None:
            st.warning("Bu stage için kayıt bulunamadı.")
        else:
            triplets = artifact.get("triplets", [])
            total = artifact.get("count", len(triplets))
            pipeline_exc = artifact.get("pipeline_exception_count", 0)
            ontology_flt = artifact.get("ontology_filtered_count", 0)

            if total == 0:
                st.success("Bu run'da hiçbir triplet elenmedi.")
            else:
                fcols = st.columns(3)
                fcols[0].metric("🚫 Toplam Elenen", total)
                fcols[1].metric("⚠️ Pipeline Exception", pipeline_exc)
                fcols[2].metric("🔴 Ontology Violation", ontology_flt)
    ...
    with tab4:
        artifact = get_artifact(selected_run_id, "final_triplets")
        if artifact is None:
            st.warning("Bu stage için kayıt bulunamadı.")
        else:
            triplets = artifact.get("triplets", [])
            count = artifact.get("count", len(triplets))
            filtered_count = artifact.get("filtered_count")
            ontology_filtered_count = artifact.get("ontology_filtered_count")

            count_cols = st.columns(3)
            count_cols[0].metric("✅ Final", count)
            if filtered_count is not None:
                count_cols[1].metric("⚠️ Filtered", filtered_count)
            if ontology_filtered_count is not None:
                count_cols[2].metric("🚫 Ontology Filtered", ontology_filtered_count)
...
# --- Ontology Neighborhood Panel ---
def render_ontology_neighborhood_panel(selected_run_id: str):
    """
    Seçili run'ın final triplets'indeki entity type'larını listeler.
    Kullanıcı bir type seçince 1-hop ontoloji neighborhood gösterir.
    """
    st.subheader("🗺️ Ontoloji Neighborhood")

    # Final triplet'lerden entity type'ları topla
    entity_types = []
    if selected_run_id:
        artifact = get_artifact(selected_run_id, "final_triplets")
        if artifact:
    ...
    if not entity_types:
        st.info("Gösterilecek entity type bulunamadı. Önce bir extraction yapın.")
        return
    ...
    if selected_type:
        ...
        # Özet metrikler
        ncols = st.columns(3)
        ncols[0].metric("🔵 Merkez Type", center["label"])
        ncols[1].metric("🟠 Parent Sayısı", len(parents))
        ncols[2].metric("🟢 Property Sayısı", len(properties))
        ...
        with detail_col1:
            ...
            if parents:
                df_parents = pd.DataFrame(parents)[["label", "id"]]
                df_parents.columns = ["Label", "Wikidata ID"]
        ...
        with detail_col2:
            ...
            if properties:
                df_props = pd.DataFrame(properties)[["label", "id", "direction"]]
                df_props.columns = ["Label", "Wikidata ID", "Direction"]
...
# Two columns: left for predefined texts, right for text area
...
if recent_runs:
    ...
    default_index = 0
    if last_run_id and last_run_id in run_ids:
        default_index = run_ids.index(last_run_id)
